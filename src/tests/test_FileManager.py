...
def test_loadDataFromOfxFile_returns_file_transactions_data():
    with open(f'tests/extratos/Extrato-01-09-2024-a-01-10-2024 (1).ofx', 'rb') as file:
        file_bytes = file.read()

        transactions_data = loadDataFromOfxFile(content = file_bytes)
...
@pytest.mark.asyncio
async def test_loadDataFromCsvFile_returns_file_transactions_data():
    mock_csv_data_list = [
        {
...
""",
    ...
    for mock_csv_data in mock_csv_data_list:
        file_content = mock_csv_data['csv_data'].encode('utf-8')
            
        transactions_data = await loadDataFromCsvFile(content = file_content, headers = mock_csv_data['headers'])
        assert isinstance(transactions_data, list)
...
@pytest.mark.asyncio
async def test_loadDataFromCsvFile_when_data_is_empty():

    empty_csv_data = "Data Lançamento;Valor;Histórico;Descrição\n".encode('utf-8')
    
    with pytest.raises(Exception) as error:
        await loadDataFromCsvFile(content = empty_csv_data)
